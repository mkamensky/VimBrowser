...
sub text {
    my $self = shift;
    my $pre = shift;
    my $text = Encode::is_utf8($pre) ? $pre : decode_utf8($pre);
...
sub formatSubtree {
    ...
    if ( ref $node ) {
        ...
        Vim::debug("processing '$tag' tag start ...", 4);
        my $goon = $self->can($func) ? (eval { $self->$func($node) } || $@) : 1;
        Vim::debug("  Going on: $goon", 4);
        if ( $goon ) {
            $self->formatSubtree($_) foreach $node->content_list;
            $func = "${tag}_end";
            Vim::debug("processing '$tag' tag end ...", 4);
            eval { $self->$func($node) } if $self->can($func);
        }
    } else {
        # got text
        Vim::debug("Processing text: $node", 4);
...
sub textflow {
    my $self = shift;
    Vim::debug("Entered textflow, line is '$_[0]'", 4);
