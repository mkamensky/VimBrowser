...
sub new {
    ...
    if ( $response ) {
        ...
        (my $encoding, $self->source) = VIM::Browser::decoded($response, 1);
        $self->header = { 
...
sub doCommand {
    my $cmd = shift;
    if ( not defined($cmd) ) {
        warn "Empty vim command" if $Vim::Option{'verbose'};
        return;
    }
...
# A variable to temporary hold the referring uri
my $Referer = '';
...
# return a decoded content of a response
sub decoded {
    my $response = shift;
    my $nowarning = shift;
    $@=undef;
    ...
    if ( $@ ) {
        ...
        Vim::warning(<<EOF) unless $nowarning;
...
EOF
        ...
        $encoding = $fallback;
        if ( $@ ) {
            Vim::warning("Failed to decode, displaying undecoded content") 
                unless $nowarning;
            $html = $response->decoded_content(charset => 'none');
            $encoding = '';
        }
    }
...
sub fetch {
    ...
    if ( not ref($req) or $req->isa('URI') ) {
        $uri = $req;
        msg("Fetching $uri...");
        $response = $Agent->get($uri, referer => $Referer);
    } else {
        $uri = $req->uri;
...
#   0. if there is a 'refresh' header, return 'refresh' as the action, with 
#   the uri as an extra argument
...
sub getAction {
    ...
    if ( $refresh and $refresh =~ /(\d+);URL=(.*)/ ) {
        # we don't currently do anything with the delay
        return ('refresh', $2, @_);
    }
...
our %FORMAT = (
    'text/html' => sub {
        my $response = shift;
        my %hints = @_;
        my ($encoding, $html) = decoded($response);
        return [] unless $html;
...
# xhtml+xml is the same from our point of view
$FORMAT{'application/xhtml+xml'} = $FORMAT{'text/html'};
...
sub follow {
    ...
    if ($link) {
        my $page = $CursorWin->page;
        $Referer = $page->uri;
        if ( my $target = $page->linkTarget($link) ) {
